...
/**
 * Get invoice status badge color for string status
 */
export function getInvoiceStatusColor(status?: string): string {
    ...
    if (
        statusLower.includes("đã phát hành") ||
...
/**
 * Get simplified invoice status text based on InvoiceStatus enum
 */
export function getSimplifiedInvoiceStatusText(status: number): string {
    // Using InvoiceStatus.Draft = 0 from enum
    if (status === 0) return "Bản nháp";
    return "Đã ghi nhận";
}

/**
 * Get simplified invoice status color class based on InvoiceStatus enum
 */
export function getSimplifiedInvoiceStatusColor(status: number): string {
    // Using InvoiceStatus.Draft = 0 from enum
...
export function formatPhoneNumber(phone: string): string {
    ...
    if (cleaned.length === 10 && cleaned.startsWith("0")) {
        ...
        return `+84 ${cleaned.slice(2, 5)} ${cleaned.slice(
            5,
            8
        )} ${cleaned.slice(8)}`;
    }
