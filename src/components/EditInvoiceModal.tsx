...
import type { UpdateInvoiceRequest, CreateInvoiceLineRequest, Invoice, InvoiceLine } from '../types/invoice';
import { getOrganizationByMe } from '../api/services/organizationService';
import { updateInvoice } from '../api/services/invoiceService';
...
const EditInvoiceModal: React.FC<EditInvoiceModalProps> = ({
    ...
    useEffect(() => {
        if (isOpen && invoice) {
            ...
            const editableLines: CreateInvoiceLineRequest[] = invoice.lines.map((line: InvoiceLine) => ({
    ...
    const handleInputChange = (field: keyof UpdateInvoiceRequest, value: any) => {
        setFormData((prev: UpdateInvoiceRequest) => ({
            ...prev,
            [field]: value
        }));
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (response.succeeded && response.data) {
                ...
                toaster.push(
                    <Message type="error" showIcon>
                        Có lỗi xảy ra khi cập nhật hóa đơn: {response.message || 'Unknown error'}
                    </Message>
                );
            }
        } catch (error) {
        ...
        >
            ...
            <Modal.Body>
                <Form
                    ...
                    model={model}
                    formValue={formData}
                    onChange={(formValue) => setFormData(formValue as UpdateInvoiceRequest)}
                    formError={formError}
                ...
                >
                    ...
                    >
                        <div style={{ padding: '16px' }}>
                            ...
                            {(attachedFiles.length > 0 || formData.attachmentFileIds?.length) && (
                                <div style={{
                                    padding: '16px',
                                    ...
                                    <p style={{
                                        ...
                                        Tệp đã tải ({(attachedFiles.length + (formData.attachmentFileIds?.length || 0))})
                                    </p>
            ...
            <Modal.Footer>
                <ButtonToolbar style={{ gap: '12px' }}>
                    ...
                    >
                        {loading ? 'Đang cập nhật...' : 'Cập nhật hóa đơn'}
                    </Button>
                </ButtonToolbar>
            </Modal.Footer>
        </Modal>
