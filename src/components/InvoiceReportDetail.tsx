...
import { Button, Message, toaster, Modal, Table, IconButton } from 'rsuite';
...
export default function InvoiceReportDetail({ report, open, onClose, onStatusChange }: Props) {
    ...
    useEffect(() => {
        if (open && report?.invoiceId) {
            setLoadingInvoice(true);
            setShowComparison(false);
            setComparisonData(null);
            getInvoiceById(report.invoiceId)
                .then(response => {
    ...
    return (
        <Modal open={open} onClose={onClose} size="lg" style={{ overflowX: 'hidden', overflowY: 'auto', maxHeight: '90vh' }}>
            ...
            <Modal.Body style={{ overflowX: 'auto', padding: '1.5rem' }}>
                {loadingInvoice ? (
