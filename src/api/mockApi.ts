 ...
 * Đăng nhập user
 */
export async function loginApi(loginData: LoginRequest): Promise<ApiResponse<AuthResponse>> {
  await simulateNetworkDelay();
  
  // Mock validation
  if (!loginData.email || !loginData.password) {
    return createErrorResponse("Email và mật khẩu là bắt buộc");
  }
 ...
 * Đăng ký user mới
 */
export async function registerApi(registerData: RegisterRequest): Promise<ApiResponse<AuthResponse>> {
  await simulateNetworkDelay();
  
  // Mock validation
  if (!registerData.email || !registerData.password || !registerData.name) {
    return createErrorResponse("Email, mật khẩu và tên là bắt buộc");
  }
 ...
 * Quên mật khẩu
 */
export async function forgotPasswordApi(email: string): Promise<ApiResponse<{ message: string }>> {
  await simulateNetworkDelay();
 ...
 */
export async function searchByCodeApi(code: string): Promise<ApiResponse<Invoice | null>> {
  await simulateNetworkDelay();
 ...
 */
export async function searchByContactApi(query: string): Promise<ApiResponse<Invoice[]>> {
  await simulateNetworkDelay();
 ...
 */
export async function uploadXmlFileApi(file: File): Promise<ApiResponse<Invoice>> {
 ...
 */
export async function getInvoicesApi(page = 1, limit = 10, status?: string): Promise<ApiResponse<PaginatedResponse<Invoice>>> {
  await simulateNetworkDelay();
  ...
  return createApiResponse(response, true, "Lấy danh sách invoice thành công");
}
...
export async function getUserProfileApi(): Promise<ApiResponse<User>> {
  ...
  return createApiResponse(mockUsers[0], true, "Lấy thông tin profile thành công");
}
...
export async function getCompanyInfoApi(): Promise<ApiResponse<Company>> {
  ...
  return createApiResponse(mockCompany, true, "Lấy thông tin công ty thành công");
}
 ...
 */
export async function getDashboardStatsApi(): Promise<ApiResponse<DashboardStats>> {
  await simulateNetworkDelay();
  
  return createApiResponse(mockDashboardStats, true, "Lấy thống kê dashboard thành công");
}
...
export async function searchByContact(query: string): Promise<Invoice[] | null> {
