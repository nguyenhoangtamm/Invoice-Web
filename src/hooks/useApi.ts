...
export function usePaginatedApi<T>(
  apiCall: (page: number, limit: number, ...args: any[]) => Promise<ApiResponse<PaginatedResponse<T>>>,
  initialPage = 1,
  initialLimit = 10,
  dependencies: any[] = []
) {
...
export function useInvoiceSearch() {
  ...
  const searchByCode = async (code: string) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setError(response.message || 'Tìm kiếm thất bại');
        setSearchResults([]);
      }
    } catch (err) {
  ...
  const searchByContact = async (query: string) => {
    ...
    try {
      const response = await apiClient.searchInvoiceByContact(query);
...
export function useInvoices(status?: string) {
  return usePaginatedApi(
    (page: number, limit: number) => apiClient.getInvoices(page, limit, status),
    1,
    10,
    [status]
  );
